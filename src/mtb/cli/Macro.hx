			...
			for(field in cls.fields.get()) if(field.isPublic) {
				...
				function addFlag(names:Array<String>, aliases:Array<Int>, isRequired:Bool) {
					...
					switch [usedName, usedAlias]  {
						...
						case [null, v]: field.pos.makeFailure('Duplicate flag alias: "-' + String.fromCharCode(v) + '". By default mtb_cli uses the first letter of the flag Rename the alias with @:alias(<single-char>) or disable alias with @:alias(false).').sure();
